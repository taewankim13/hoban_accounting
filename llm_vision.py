...
def analyze_receipt_with_llm(image_path: str, api_key=None) -> dict:
...
예: 차임(월세), 보증금, 월세액, 임대료, 부가세액, 공급가액, 납품장소, 하자보증기간, 지체상금률, 계약보증금, 선급금, 대금지급조건, 특약사항 등
...
def parse_evidence_document(image_path: str, evidence_type: str, api_key=None) -> dict:
...
추가 항목 (이미지에서 발견되는 경우 자유롭게 추가):
예: 거래처명, 거래처코드, 계약금액, 도급금액, 도급계약금액, 발주처, 발주자, 발주기관, 기성비율, 전회기성, 금회기성, 누계기성, 잔액, 선급금, 준공일자, 착공일자, 검수일자, 납품장소, 품목, 수량, 단가, 결재선, 협조부서, 비고, 특기사항 등
...
- "기성비율" (기성률 %)도 추출하세요
- "도급금액" (도급계약금액/계약금액)도 추출하세요
- "발주처" (발주자/발주기관)도 추출하세요
...
def parse_linked_document(image_path: str, api_key=None) -> dict:
