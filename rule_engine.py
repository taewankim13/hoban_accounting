...
# ──────────────────────────────────────
# AI 검토 제안 생성 (템플릿 + LLM 하이브리드)
# ──────────────────────────────────────

RULE_SUGGESTION_TEMPLATES = {
    "E001": {
        "name": "차대변 불일치",
        "check": "차변 합계와 대변 합계 금액이 정확히 일치하는지 확인합니다.",
        "fix": "차변/대변 금액을 재계산하고, 누락된 라인 항목이 없는지 확인합니다. 단수 차이 시 잡이익/잡손실 계정으로 조정합니다.",
        "steps": "1) ERP 원본 전표와 대사 → 2) 누락 라인 확인 → 3) 금액 오입력 여부 점검 → 4) 수정 전표 기표"
    },
    "E002": {
        "name": "계정과목 착오 의심",
        "check": "적요 내용과 입력된 계정과목이 일치하는지 확인합니다. 회계기준상 적정 계정과목을 검토합니다.",
        "fix": "적요에 기재된 거래 성격에 맞는 계정과목으로 수정합니다. 예: 월세→임차료, 식대→복리후생비(일반).",
        "steps": "1) 적요 내용 확인 → 2) 회사 계정과목 기준표 참조 → 3) 올바른 계정코드로 수정분개 → 4) 원천징수 영향 여부 확인"
    },
    "E003": {
        "name": "중복 전표 의심",
        "check": "동일 일자, 동일 금액, 동일 거래처/계정으로 기표된 다른 전표가 있는지 확인합니다.",
        "fix": "중복 확인 후 실제 중복이면 하나를 역분개 처리합니다. 동일 거래처에 대한 별건 거래인 경우 적요에 구분 사유를 기재합니다.",
        "steps": "1) 동일 조건 전표 목록 조회 → 2) 세금계산서/영수증 대조 → 3) 중복 확정 시 역분개 → 4) 거래처 확인서 징구"
    },
    "E004": {
        "name": "고액 전표",
        "check": "거래 금액이 기준액을 초과합니다. 증빙자료와 승인 절차를 확인합니다.",
        "fix": "계약서, 세금계산서 등 증빙서류가 첨부되어 있는지 확인하고, 결재 권한에 따른 승인 여부를 점검합니다.",
        "steps": "1) 금액 적정성 확인 → 2) 계약서/발주서 대조 → 3) 결재 권한 기준 확인 → 4) 증빙서류 보완"
    },
    "E005": {
        "name": "주말/공휴일 기표",
        "check": "주말 또는 공휴일에 기표된 전표입니다. 긴급 기표 사유가 있는지 확인합니다.",
        "fix": "업무상 불가피한 사유(월말 마감, 긴급 결제 등)가 있으면 적요에 사유를 기재합니다.",
        "steps": "1) 기표일이 주말/공휴일인지 재확인 → 2) 입력자에게 기표 사유 확인 → 3) 긴급 기표 사유서 징구 → 4) 향후 익영업일 기표 안내"
    },
    "E006": {
        "name": "역분개 패턴",
        "check": "기존 전표와 동일 금액이 반대 방향으로 기표되었습니다. 정당한 역분개인지 확인합니다.",
        "fix": "수정분개, 반품, 계약 해지 등 정당한 사유 확인. 적요에 역분개 사유 및 원전표 번호를 기재합니다.",
        "steps": "1) 원전표 확인 → 2) 역분개 사유 확인 (반품/수정/해지) → 3) 관련 증빙 첨부 → 4) 적요에 원전표번호 기재"
    },
    "E007": {
        "name": "증빙 누락",
        "check": "전표에 증빙자료(세금계산서, 영수증 등)가 첨부되어 있지 않습니다.",
        "fix": "거래에 해당하는 증빙서류를 입수하여 첨부합니다.",
        "steps": "1) 거래 유형에 필요한 증빙 종류 확인 → 2) 거래처에 증빙 요청 → 3) 증빙 수취 후 시스템에 첨부 → 4) 수취 불가 시 지출결의서 작성"
    },
    "E008": {
        "name": "차대변 방향 오류",
        "check": "계정과목의 정상적인 차/대변 방향과 다르게 입력되었습니다.",
        "fix": "계정과목별 정상 차대변 방향을 확인하고 수정합니다. 역분개/수정분개의 경우 적요에 사유를 기재합니다.",
        "steps": "1) 계정과목 차대변 기준 확인 → 2) 정당한 반대 방향 기표 사유 확인 → 3) 오입력 시 수정분개 → 4) 적요에 사유 명기"
    },
    "E009": {
        "name": "월말 집중 기표",
        "check": "월말(25일 이후)에 집중적으로 기표된 전표입니다.",
        "fix": "실제 거래 발생일과 기표일이 일치하는지 확인합니다. 기간귀속이 적정한지 점검합니다.",
        "steps": "1) 실제 거래일 확인 → 2) 기간 귀속 적정성 점검 → 3) 월말 마감 기표인 경우 사유 확인"
    },
    "E010": {
        "name": "단수/라운드 금액",
        "check": "금액이 정확한 만원, 십만원 단위의 라운드 넘버입니다. 실제 거래 금액인지 확인합니다.",
        "fix": "세금계산서/영수증의 실제 금액과 대조하여 정확한 금액을 입력합니다.",
        "steps": "1) 원본 증빙서류 금액 확인 → 2) 예정가/개산급 여부 확인 → 3) 정산 후 차액 조정 필요 여부 점검"
    },
    "E011": {
        "name": "근무시간 외 입력",
        "check": "근무시간 외(18시 이후~08시 이전)에 입력된 전표입니다.",
        "fix": "야근, 긴급 업무 등 정당한 사유가 있는지 확인합니다.",
        "steps": "1) 입력 시각 확인 → 2) 입력자에게 사유 확인 → 3) 대리 입력 여부 점검"
    },
    "E012": {
        "name": "개인거래처 고액",
        "check": "개인(P) 유형 거래처에 대한 고액 거래입니다. 원천징수 및 계약 관계를 확인합니다.",
        "fix": "용역계약서, 원천징수 영수증 등을 확인합니다. 사업자 등록 여부를 재점검합니다.",
        "steps": "1) 거래처 유형(개인/법인) 재확인 → 2) 계약서/용역 내역 확인 → 3) 원천징수 이행 여부 점검 → 4) 사업자 미등록 시 사업소득 원천징수"
    },
    "E013": {
        "name": "적요 미입력",
        "check": "전표의 적요(거래 설명)가 비어 있습니다.",
        "fix": "거래 내용을 파악하여 적요를 입력합니다.",
        "steps": "1) 증빙서류로 거래 내용 파악 → 2) 구체적인 적요 작성 (거래처, 내용, 기간 등 포함)"
    },
    "E014": {
        "name": "다건 분개 항목",
        "check": "하나의 전표에 분개 항목이 많습니다. 전표 분리 또는 합산 적정성을 확인합니다.",
        "fix": "관련 없는 거래가 하나의 전표에 합산되어 있다면 별도 전표로 분리합니다.",
        "steps": "1) 각 라인 항목의 거래 관련성 확인 → 2) 미관련 거래 분리 기표 → 3) 합산 기표 사유 확인"
    },
    "E015": {
        "name": "반복거래 패턴 이탈",
        "check": "동일 거래처에 대해 통상적으로 사용하는 계정과목과 다른 계정이 사용되었습니다.",
        "fix": "해당 거래처와의 계약 내용 변경이 있는지 확인합니다. 단순 착오 시 수정합니다.",
        "steps": "1) 거래처별 과거 기표 이력 조회 → 2) 계정과목 변경 사유 확인 → 3) 착오 시 정상 계정으로 수정분개"
    },
}


def generate_template_suggestion(error_codes: list, journal_info: dict) -> str:
    """룰 코드 기반 사전 정의 템플릿으로 검토 제안을 즉시 생성한다. LLM 호출 없음."""
    parts = []
    doc_no = journal_info.get("doc_no", "")
    parts.append(f"### 전표 {doc_no} AI 검토 제안\n")

    for code in error_codes:
        tmpl = RULE_SUGGESTION_TEMPLATES.get(code)
        if tmpl:
            parts.append(f"#### [{code}] {tmpl['name']}\n")
            parts.append(f"1. **점검 포인트**: {tmpl['check']}")
            parts.append(f"2. **수정 방법**: {tmpl['fix']}")
            parts.append(f"3. **검토 절차**: {tmpl['steps']}\n")
        else:
            parts.append(f"#### [{code}] 탐지된 이상 항목\n")
            parts.append("1. **점검 포인트**: 해당 위반 사항의 원인을 파악합니다.")
            parts.append("2. **수정 방법**: 증빙서류와 대조하여 오류 여부를 확인합니다.")
            parts.append("3. **검토 절차**: 관련 증빙 확인 → 입력자 확인 → 필요 시 수정분개\n")
