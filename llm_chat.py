"""Claude API 기반 전표 어시스턴트 - 대화형 전표 생성/수정 지원"""
import os
import json
import re
...
def chat_with_claude(messages: list, current_form: dict = None, api_key=None) -> dict:
    """Claude API로 대화형 전표 어시스턴트를 실행한다."""

    # 현재 폼 상태를 시스템 프롬프트에 추가
    ...
    if api_key:
        try:
            import anthropic
            active_client = anthropic.Anthropic(api_key=api_key)
        except Exception:
            active_client = client
    if not active_client:
        return {"message": "Anthropic API 키가 설정되지 않았습니다.", "error": "NO_API_KEY"}
...
def chat_with_local(message: str, current_form: dict = None) -> dict:
    ...
    # account_master.json에서 계정 매핑 로드
    ...
    # parse_chat_input 실패 시 더 친절한 메시지
    if not result.get("success"):
        ...
        result["success"] = True  # UI에서 에러가 아닌 안내로 표시

    return result


def process_chat(message: str, history: list = None, current_form: dict = None, api_key=None) -> dict:
    """챗 메시지를 처리한다. Claude API가 있으면 사용, 없으면 로컬 파서."""

    if api_key or HAS_CLAUDE:
        # Claude API 사용
        messages = []
        for h in (history or []):
            messages.append({"role": h["role"], "content": h["content"]})
        messages.append({"role": "user", "content": message})
        return chat_with_claude(messages, current_form, api_key=api_key)
    else:
        # 로컬 강화 파서
